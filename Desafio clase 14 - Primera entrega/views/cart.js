            ...
            $("#carritos").append(
                `
                    <div class="alert alert-primary" role="alert">
                        Carrito # ${carrito.id}, creado el ${carrito.timestamp}
...
function addToCart(){
    ...
    const idProd = $("#idProducto").val()
     ...
     fetch('/api/carrito')
     .then( response => response.json())
     .then(data => { carritos=data

      let cartId = carritos.findIndex((objT) => objT.id == idCart)
       if(cartId!=-1) {
      fetch('/api/productos/'+idProd)   
      .then(response => response.json())         
      .then(data => { productos=data 

             if((productos.id!=undefined) && (productos.stock!=0)) {
                 let oldStock = productos.stock
                 productos.stock=1  
                 multiusageFetch(idCart, "POST", productos, "Objeto agregado exitosamente al carrito.")
                 productos.stock=oldStock-1 
                 fetch('/api/productos/'+idProd, {
                    method: "PUT",
                    headers: {
